...
class ScreenShootResilient(pl.LightningModule):
    ...
    def __init__(self, region_selector: nn.Module,
        ...
        log.info(f'Encoder Summary:\n'
                 f'{summary(self.encoder, input_size=((1, 3, 256, 256), (1, 3, 256, 256)), depth=5, verbose=0)}')
    ...
    def shared_step(self, batch: Any):
        ...
        aug_container = self.augmenter(container, current_batch_idx, {'illumination': illumination_pattern,
                                                                      'moire': moire_pattern})
        ...
        secret_hat_logit = self.decoder(aug_container, normalize=True)
        return host, secret, region, encoded_region, container, aug_container, secret_gt, secret_hat_logit
        ...
        if optimizer_idx == 0:
            ...
            if current_batch_idx % 3000 == 0:
                show_image = dict(
                    ...
                    redisual=K.enhance.normalize_min_max(encoded_region[0]-region[0]),
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        ...
        secret_hat = secret_hat_logit.sigmoid().ge(0.5).float()
        ...
        if batch_idx == 0:
            show_image = dict(
                region=region[0],
                encoded_region=encoded_region[0],
                residual=K.enhance.normalize_min_max(encoded_region[0]-region[0]),
                container=container[0],
                aug_container=aug_container[0],
                secret_gt=secret_gt[0],
