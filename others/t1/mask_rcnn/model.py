...
from mask_rcnn.nms.nms_wrapper import nms
from mask_rcnn.roialign.roi_align.crop_and_resize import CropAndResizeFunction
...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    if torch.nonzero(positive_roi_bool).size():
        ...
        deltas = Variable(mask_rcnn.utils.box_refinement(positive_rois.data, roi_gt_boxes.data), requires_grad=False)
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    shape = image.shape
    image, window, scale, padding = mask_rcnn.utils.resize_image(
        image,
        min_dim=config.IMAGE_MIN_DIM,
        max_dim=config.IMAGE_MAX_DIM,
        padding=config.IMAGE_PADDING)
    mask = mask_rcnn.utils.resize_mask(mask, scale, padding)
    ...
    bbox = mask_rcnn.utils.extract_bboxes(mask)
    ...
    if use_mini_mask:
        mask = mask_rcnn.utils.minimize_mask(bbox, mask, config.MINI_MASK_SHAPE)
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    if crowd_ix.shape[0] > 0:
        ...
        # Compute overlaps with crowd boxes [anchors, crowds]
        crowd_overlaps = mask_rcnn.utils.compute_overlaps(anchors, crowd_boxes)
    ...
    overlaps = mask_rcnn.utils.compute_overlaps(anchors, gt_boxes)
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, dataset, config, augment=True):
        ...
        self.anchors = mask_rcnn.utils.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES,
...
class MaskRCNN(nn.Module):
    ...
    def build(self, config):
        ...
        # Generate Anchors
        self.anchors = Variable(torch.from_numpy(mask_rcnn.utils.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES,
                                                                                config.RPN_ANCHOR_RATIOS,
                                                                                config.BACKBONE_SHAPES,
                                                                                config.BACKBONE_STRIDES,
    ...
    def forward(self, images):
        images = [np.random.rand(400, 400, 3)]
    ...
    def detect(self, images):
        ...
        self.predict([molded_images, image_metas], mode='inference')
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            molded_image, window, scale, padding = mask_rcnn.utils.resize_image(
                image,
    ...
    def unmold_detections(self, detections, mrcnn_mask, image_shape, window):
        ...
        for i in range(N):
            ...
            full_mask = mask_rcnn.utils.unmold_mask(masks[i], boxes[i], image_shape)
